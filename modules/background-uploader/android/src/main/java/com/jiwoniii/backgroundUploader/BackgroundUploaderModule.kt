package com.jiwoniii.backgroundUploader
...
class BackgroundUploaderModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    Function("startUpload") { threadsJson: String ->
      ...
      if (!receiverRegistered) {
        val filter = IntentFilter("com.jiwoniii.backgroundUploader.UPLOAD_FINISHED")
  ...
  private val eventReceiver = object : BroadcastReceiver() {
    override fun onReceive(context: Context?, intent: Intent?) {
      if (intent?.action == "com.jiwoniii.backgroundUploader.UPLOAD_FINISHED") {
